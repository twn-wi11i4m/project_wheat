...
def universal_approach_3_years_mpt_short(
    ...
    if return_farmer_only:
        ...
        fig = plot_portfolio(farmer, date_list, res_path='res_html', title=f'approach_mpt_short', sub_title=f"approach 2018 ratio: {d1}, 2019 ratio: {d2}, 2020 ratio:{d3} with mpt (with short) method: {mpt_config['method']} | Financial Product: {financial_product_selection}", benchmark_farmer=benchmark_farmer)
    return fig
